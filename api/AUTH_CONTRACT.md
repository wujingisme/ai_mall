...
- 第一版支持用户名 + 密码以及微信小程序登录，不包含短信登录和找回密码；自助注册及首次微信登录账号固定为 `CUSTOMER`，不能由客户端指定角色。
- 微信小程序端仅提交 `wx.login`/`uni.login` 返回的一次性 `code`；AppSecret 只允许由后端环境变量提供，禁止进入前端包、Git 仓库、日志或接口响应。
...
- 微信登录由后端调用微信 `code2Session`，以同一小程序内唯一的 OpenID 查找用户；首次登录创建用户，再次登录必须复用原用户。
- 微信适配层不得依赖外部响应的 `Content-Type` 自动反序列化：必须限长读取原始响应、独立解析并校验 `errcode`/`openid`。无效 code 返回 401；未配置、上游不可用、非标准或非法响应返回 503，且不得记录或透传 AppSecret、code、完整 URL、原始响应和外部 `errmsg`。
...
7. 微信有效 code 首次登录创建 CUSTOMER，再次登录复用同一用户；空、无效、过期或重复使用的 code 返回稳定错误，禁用用户返回 403。
