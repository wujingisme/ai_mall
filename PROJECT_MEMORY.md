...
- Admin order query UI is available at `/orders`, backed by `/api/v1/admin/orders` and `/api/v1/admin/orders/{id}`; it supports status/order-number/customer-ID filters, server pagination, detail drawer, and snapshot item display. Pickup verification and inventory settlement are not exposed in the page yet.
...
- Order pickup design — 2026-09-04: added `公共知识/订单功能设计方案.md` for online ordering with offline pickup; excludes logistics and real payment, assumes one pickup point, defines order/item snapshots, reserved inventory, one-time pickup codes, state transitions, APIs, phased implementation, and local acceptance cases. Phase 1 repository implementation is complete: `mall_order`/`order_item` DDL and migration, preview API, and read-only “my orders” APIs are present; the frontend has typed order API clients, cart-to-preview flow, and list/detail pages. Phase 2 inventory foundation, formal creation, and consumer submission UI are implemented; Phase 3 cancellation/release is now also implemented; the first Admin substeps (backend order list/detail queries with admin authorization and the basic Admin order list/detail page) are complete. The three order migrations have been executed locally; other environments still need `20260904_order_base.sql`, `20260904_order_inventory.sql`, and `20260904_order_create.sql`. Pickup verification/settlement and coupon redemption remain future work.
