...
- Admin user management is available at `/users`, backed by `/api/v1/admin/customers/manage`; it lists/searches users with the `CUSTOMER` role, including deliberate `ADMIN,CUSTOMER` dual-role accounts, shows only a `wechatBound` boolean instead of OpenID, and exposes coupon summaries. Pure customer accounts support activation/deactivation; mixed staff accounts are visible but backend and frontend both refuse customer enable/disable actions so staff access cannot be accidentally disabled. All operations require an admin role.
- A logged-in backend role can opt into consumer use through `POST /api/v1/admin/accounts/me/customer-role`; it only modifies the current JWT user's roles, preserves existing staff roles, is idempotent, and requires a fresh frontend login to receive the new CUSTOMER claim.
- Admin order query UI is available at `/orders`, backed by `/api/v1/admin/orders` and `/api/v1/admin/orders/{id}`; it supports status/order-number/customer-ID filters, server pagination, detail drawer, and snapshot item display. The backend pickup endpoint is now available at `POST /api/v1/admin/orders/{id}/pickup-verification`, but the Admin page still needs its verification button and modal.
...
- Order pickup design — 2026-09-04: added `公共知识/订单功能设计方案.md` for online ordering with offline pickup; excludes logistics and real payment, assumes one pickup point, defines order/item snapshots, reserved inventory, one-time pickup codes, state transitions, APIs, phased implementation, and local acceptance cases. Phase 1 repository implementation is complete: `mall_order`/`order_item` DDL and migration, preview API, and read-only “my orders” APIs are present; the frontend has typed order API clients, cart-to-preview flow, and list/detail pages. Phase 2 inventory foundation, formal creation, and consumer submission UI are implemented; Phase 3 cancellation/release, Admin order list/detail queries, the basic Admin order page, and backend pickup verification/settlement are implemented. The pickup endpoint validates the hash, locks the order and products, atomically transfers `stock/reserved_stock`, and makes correct repeats idempotent. The three order migrations have been executed locally; other environments still need `20260904_order_base.sql`, `20260904_order_inventory.sql`, and `20260904_order_create.sql`. The Admin verification button and coupon redemption remain future work.
...
- `admin`: `npm run type-check` and `npm run build` succeed. The Admin `/orders` route now supports order filters, pagination, detail drawer, and snapshot item display. The header provides a protected “开通前台身份” action for staff accounts without CUSTOMER; the `/users` table displays roles and does not offer staff-account enable/disable. Vite reports a non-blocking large-chunk warning (the main JS bundle is over 500 kB); browser runtime acceptance of the new controls is still pending.
...
- `backend`: Maven test suite succeeds with 79 tests when using the repository-local Maven cache command above. Order Phase 1 through Phase 3 cancellation, Admin order query, backend pickup verification, and dual-role opt-in contracts were verified with local tests, including wrong-code rejection, repeat-code idempotency, inventory rollback, anonymous access denial, role-append idempotency, and staff-account disable protection. The earlier consumer order runtime flow and cancellation flow were verified against local MySQL on alternate port 18080 after applying the three order migrations; pickup verification and dual-role opt-in have not yet had real browser/API runtime acceptance. Port 8080 may still be occupied by an existing local backend process, so alternate-port verification remains useful.
...
- Use the Admin header’s “开通前台身份” action for an administrator who must also shop; after success, log in to the consumer client again so its JWT contains `CUSTOMER`, then confirm the account appears in `/users` with both roles.
