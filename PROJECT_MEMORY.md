...
- `admin`: `npm run type-check` and `npm run build` succeed. The Admin `/orders` route now supports order filters, pagination, detail drawer, and snapshot item display. Vite reports a non-blocking large-chunk warning (the main JS bundle is over 500 kB); browser runtime acceptance of the new page is still pending.
...
- `backend`: Maven test suite succeeds with 73 tests when using the repository-local Maven cache command above. Order Phase 1 through Phase 3 cancellation, Admin order query, and backend pickup verification contracts were verified with local tests, including wrong-code rejection, repeat-code idempotency, inventory rollback, and anonymous access denial. The earlier consumer order runtime flow and cancellation flow were verified against local MySQL on alternate port 18080 after applying the three order migrations; pickup verification has not yet had a real browser/API runtime acceptance. Port 8080 may still be occupied by an existing local backend process, so alternate-port verification remains useful.
