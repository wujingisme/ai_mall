...
export default function AdminUsersPage() {
  ...
  // 双角色账号会出现在客户列表，但不能被客户启停操作禁用，否则会连后台登录一起关闭。
  const isStaffAccount = (user: AdminUser) => user.roles.split(',').some((role) => ['SUPER_ADMIN', 'ADMIN', 'OPERATOR'].includes(role.trim()));
  const columns: ColumnsType<AdminUser> = [
    ...
    { title: '角色', dataIndex: 'roles', render: (value) => value.split(',').map((role: string) => <Tag key={role}>{role}</Tag>) },
    { title: '注册时间', dataIndex: 'createdAt', render: (value) => value ? new Date(value).toLocaleString('zh-CN', { hour12: false }) : '-' },
    { title: '操作', render: (_, row) => <Space><Button type="link" icon={<EyeOutlined />} onClick={() => open(row)}>查看</Button>{isStaffAccount(row) ? <Typography.Text type="secondary">后台账号</Typography.Text> : <Button type="link" danger={row.enabled} onClick={() => toggle(row)}>{row.enabled ? '停用' : '启用'}</Button>}</Space> },
  ];
